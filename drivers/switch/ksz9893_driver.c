...
void ksz9893SetAgingTime(NetInterface *interface, uint32_t agingTime)
{
   ...
   temp = ksz9893ReadSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL3);
   ...
   ksz9893WriteSwitchReg8(interface, KSZ9893_SWITCH_LUE_CTRL3, temp);
}
...
void ksz9893FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   {
      //Loop through the ports
      for(i = KSZ9893_PORT1; i <= KSZ9893_PORT3; i++)
      {
         //Matching port number?
         if(i == port || port == 0)
         {
            //Save the current state of the port
            state[i - 1] = ksz9893ReadSwitchReg8(interface,
               KSZ9893_PORTn_MSTP_STATE(i));

            //Turn off learning capability
            ksz9893WriteSwitchReg8(interface, KSZ9893_PORTn_MSTP_STATE(port),
               state[i - 1] | KSZ9893_PORTn_MSTP_STATE_LEARNING_DIS);
         }
      }
      ...
      for(i = KSZ9893_PORT1; i <= KSZ9893_PORT3; i++)
      {
         ...
         if(i == port || port == 0)
         {
            ...
            ksz9893WriteSwitchReg8(interface, KSZ9893_PORTn_MSTP_STATE(i),
               state[i - 1]);
         }
      }
   }
