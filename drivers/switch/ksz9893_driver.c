...
{
   0,
   KSZ9893_TAIL_TAG_ENCODE(1),
   KSZ9893_TAIL_TAG_ENCODE(2)
};
...
error_t ksz9893Init(NetInterface *interface)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      do
      {
         ...
         temp = ksz9893ReadSwitchReg(interface, KSZ9893_CHIP_ID1);

         //The returned data is invalid until the serial interface is ready
      } while(temp != KSZ9893_CHIP_ID1_DEFAULT);

      //Reset switch
      ksz9893WriteSwitchReg(interface, KSZ9893_SWITCH_OP,
      ...
      do
      {
         ...
         temp = ksz9893ReadSwitchReg(interface, KSZ9893_SWITCH_OP);
      ...
      if(interface->port != 0)
      {
         ...
         temp = ksz9893ReadSwitchReg(interface, KSZ9893_PORT3_OP_CTRL0);
         temp |= KSZ9893_PORTn_OP_CTRL0_TAIL_TAG_EN;
         ksz9893WriteSwitchReg(interface, KSZ9893_PORT3_OP_CTRL0, temp);

         //Loop through ports
         for(port = KSZ9893_PORT1; port <= KSZ9893_PORT2; port++)
         {
            //Disable packet transmission and switch address learning
            temp = ksz9893ReadSwitchReg(interface, KSZ9893_PORTn_MSTP_STATE(port));
            ...
            ksz9893WriteSwitchReg(interface, KSZ9893_PORTn_MSTP_STATE(port), temp);
         }
      }
      else
#endif
      ...
      temp = ksz9893ReadSwitchReg(interface, KSZ9893_PORT3_XMII_CTRL1);
      temp |= KSZ9893_PORTn_XMII_CTRL1_RGMII_ID_IG;
      temp |= KSZ9893_PORTn_XMII_CTRL1_RGMII_ID_EG;
      ksz9893WriteSwitchReg(interface, KSZ9893_PORT3_XMII_CTRL1, temp);
...
void ksz9893Tick(NetInterface *interface)
{
...
#endif
   {
      ...
      for(port = KSZ9893_PORT1; port <= KSZ9893_PORT2; port++)
      {
         ...
         //Retrieve current link state
         if(value & KSZ9893_BMSR_LINK_STATUS)
            linkState = TRUE;
      }
            ...
            virtualInterface->parent == interface)
         {
            ...
            if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
            {
               ...
               if(linkState && !virtualInterface->linkState)
               {
                  ...
                  //Check current speed
                  if(value & KSZ9893_PHYCON_SPEED_1000BT)
                  {
                     //1000BASE-T
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_1GBPS;
                  }
                  else if(value & KSZ9893_PHYCON_SPEED_100BTX)
                  {
                     //100BASE-TX
                     virtualInterface->linkSpeed = NIC_LINK_SPEED_100MBPS;
                  }
                  else if(value & KSZ9893_PHYCON_SPEED_10BT)
                  {
                     //10BASE-T
                  ...
                  if(value & KSZ9893_PHYCON_DUPLEX_STATUS)
   ...
   size_t *offset, uint8_t port, uint16_t *type)
{
   ...
   if(port >= KSZ9893_PORT1 && port <= KSZ9893_PORT2)
   {
      ...
      tailTag = &ksz9893IngressTailTag[port];
   ...
   size_t *length, uint8_t *port)
{
   ...
   if(*length >= (sizeof(EthHeader) + sizeof(uint8_t)))
   {
      ...
      *port = KSZ9893_TAIL_TAG_DECODE(*tailTag);
   ...
   uint8_t address, uint16_t data)
{
   ...
   if(interface->spiDriver != NULL)
   {
      ...
      command |= KSZ9893_PORTn_ETH_PHY_REG(port, address) << 5;
...
void ksz9893DumpSwitchReg(NetInterface *interface)
{
   ...
   for(i = 0; i < 256; i++)
   {
      ...
      TRACE_DEBUG("0x%02" PRIX16 " (%02" PRIu16 ") : 0x%02" PRIX8 "\r\n",
         i, i, ksz9893ReadSwitchReg(interface, i));
   }
