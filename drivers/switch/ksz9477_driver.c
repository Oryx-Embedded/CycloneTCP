...
void ksz9477SetAgingTime(NetInterface *interface, uint32_t agingTime)
{
   ...
   temp = ksz9477ReadSwitchReg8(interface, KSZ9477_SWITCH_LUE_CTRL3);
   ...
   ksz9477WriteSwitchReg8(interface, KSZ9477_SWITCH_LUE_CTRL3, temp);
}
...
void ksz9477FlushDynamicFdbTable(NetInterface *interface, uint8_t port)
{
   ...
   {
      //Loop through the ports
      for(i = KSZ9477_PORT1; i <= KSZ9477_PORT7; i++)
      {
         //Matching port number?
         if(i == port || port == 0)
         {
            //Save the current state of the port
            state[i - 1] = ksz9477ReadSwitchReg8(interface,
               KSZ9477_PORTn_MSTP_STATE(i));

            //Turn off learning capability
            ksz9477WriteSwitchReg8(interface, KSZ9477_PORTn_MSTP_STATE(port),
               state[i - 1] | KSZ9477_PORTn_MSTP_STATE_LEARNING_DIS);
         }
      }
      ...
      for(i = KSZ9477_PORT1; i <= KSZ9477_PORT7; i++)
      {
         ...
         if(i == port || port == 0)
         {
            ...
            ksz9477WriteSwitchReg8(interface, KSZ9477_PORTn_MSTP_STATE(i),
               state[i - 1]);
         }
      }
   }
